...
namespace {
    ...
    int require_int_range(const Token &token, const Value &value, std::string_view description, std::int64_t minimum,
                          std::int64_t maximum) {
        const std::int64_t number = require_integer(token, value, description);

        if (number < minimum || number > maximum) {
            throw RuntimeError(token, std::string(description) + " must be in range " + std::to_string(minimum) +
